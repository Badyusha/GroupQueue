...
public interface StudentRepository extends CrudRepository<StudentEntity, Long> {
	...
	@Query(value = """
					SELECT s.username, s.firstName,
					s.lastName, role.name AS roleName,
					group.number AS groupNumber, s.password
					FROM StudentEntity s
					...
					WHERE s.id = ?1
				""")
	List<Object[]> getStudentInfoByStudentId(long studentId);
